...
def _get_abreviations() -> dict[str, str]:
    file_path = os.path.join(os.path.dirname(__file__), "static", "text", "abreviations.json")
    try:
        with open(file_path, "r", encoding='utf-8') as f:
            return json.load(f)
    except FileNotFoundError:
        print(f"Warning: {file_path} not found, using empty abbreviations")
        return {}
...
def _get_last_letters() -> dict[str, list[str]]:
    file_path = os.path.join(os.path.dirname(__file__), "static", "text", "last_letter.json")
    try:
        with open(file_path, "r", encoding='utf-8') as f:
            return json.load(f)
    except FileNotFoundError:
        print(f"Warning: {file_path} not found, using empty last letters")
        return {}
